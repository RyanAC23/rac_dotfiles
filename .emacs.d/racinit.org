...
** Visual Tweaks
Disable the default splash screen, the visual scrollbars, and the menu bar that you never click.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
* Searching
The three packages here are ~ivy~, ~counsel~, and ~swiper~. Together they give regular expression searches with spaces and suggest completions for commands and other minibuffer actions.
#+BEGIN_SRC emacs-lisp
